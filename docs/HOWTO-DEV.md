...
A quick 'n' dirty HOW-TO for Zulfiqar OS Linux developers. It contains the most
...
[Matrix](https://matrix.to/#/#Zulfiqar OS:matrix.org).
...
- CoC for Zulfiqar OS is almost the same like:
...
- If you are new try to follow other developers's work as much as possible. Talk
  to us on [Matrix](https://matrix.to/#/#Zulfiqar OS:matrix.org) for further
...
- Use our [PKGBUILD templates](https://github.com/Zulfiqar OS/zulfiqar-pkgbuilds)
...
- If you are unsure, copy an existing PKGBUILD from a package and use that as a
  ...
  directories of the zulfiqar github repository (the current repository)
...
PKGBUILDs and ensure that every package uploaded to the Zulfiqar OS Linux
...
- A Zulfiqar OS Linux system with all tools installed
  (`# pacman -Syu --needed zulfiqar`).
...
- Install the package in the full existing Zulfiqar OS Linux installation to
...
[zulfiqar-iso](https://github.com/Zulfiqar OS/zulfiqar-iso) repository. Please
...
[zulfiqar-website](https://github.com/Zulfiqar OS/zulfiqar-site) repository. It
is currently starting the process of being rebuilt, talk to noptrix and other
devs on the [Matrix](https://matrix.to/#/#Zulfiqar OS:matrix.org) for more info.

## On-Boarding for new developers

Following information are immediately needed when you want to join the Zulfiqar OS
Linux project:

- **Name**: Your name will be used on website. Say 'anon' to stay anonym.

- **E-Mail**: For the team@zulfiqar.org mailinglist. You will receive any mails
  sent to that list.

- **Handle**: For references in any Zulfiqar OS project and also for Matrix.

- **Twitter**: Your twitter name if present. @zulfiqarlinux will then follow
  you.
